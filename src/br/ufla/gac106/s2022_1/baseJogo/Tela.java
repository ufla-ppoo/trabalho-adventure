...
public class Tela implements InterfaceUsuario {
    ...
    private void criarPainelAmbiente() {
        ...
        painelAmbiente.setPreferredSize(
            new Dimension(larguraPainelCentral, 500)
        );
        painelAmbiente.setBorder(
    ...
    private void criarPainelItens() {
        ...
        painelItens.setPreferredSize(
            new Dimension(larguraJanela - larguraPainelCentral, 500)
        );
        painelItens.setBorder(
            BorderFactory.createTitledBorder("Jogador possui")
        );
    ...
    private void criarSubPainelMensagens(JPanel painelPai) {
        ...
        areaMensagensScrollPane.setPreferredSize(
            new Dimension(larguraJanela - 20, 140)
        );
        areaMensagensScrollPane.setVerticalScrollBarPolicy(
    ...
    public void ambienteAtualMudou(EntidadeGrafica ambiente) {
        ...
        if (imagem != null) {
            ...
            rotuloImagemAmbiente.setIcon(
                new ImageIcon(redimensionarImagem(ambiente.getImagem(), true))
            );
            rotuloImagemAmbiente.setText(null);
        } else {
            // se a imagem não foi carregada informa isso na tela
            rotuloImagemAmbiente.setIcon(null);
            rotuloImagemAmbiente.setText(
                "Imagem não foi carregada! Ambiente: " + ambiente.getNome()
            );
        }
    ...
    public void jogadorPegouItem(EntidadeGrafica item) {
        ...
        if (quantidade == 0) {
            ...
            if (imagem != null) {
                ...
                rotuloImagemItem =
                    new JLabel(
                        new ImageIcon(redimensionarImagem(imagem, false))
                    );
            } else {
                // se a imagem não foi carregada informa isso na tela
            ...
            adicionarComponenteDeItem(
                rotuloImagemItem,
                item.getNome() + "_img"
            );
        } else { // se o item já existia no painel de itens
            JLabel rotuloNomeItem = (JLabel) componentesDosItens.get(
                item.getNome()
            );
    ...
    private int quantidadeDoItemNoPainel(EntidadeGrafica item) {
        ...
        if (rotuloItem == null) {
            ...
            if (rotuloNomeItem.getText().equals(item.getNome())) {
                ...
                int indiceAbreParenteses = rotuloNomeItem
                    .getText()
                    .lastIndexOf("(");
                int indiceFechaParenteses = rotuloNomeItem
                    .getText()
                    .lastIndexOf(")");
                return Integer.parseInt(
                    rotuloNomeItem
                        .getText()
                        .substring(
                            indiceAbreParenteses + 2,
                            indiceFechaParenteses
                        )
                );
            }
        }
